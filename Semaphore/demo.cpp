...
void RdsCbCmd_PUSH(redisAsyncContext* pRdsContext, void* pReply, void* pData);
...
//        char szValue[1024] = {0};
//        snprintf(szValue, sizeof(szValue), "%4d", i+1);
//        ppArg[2] = szValue;
//        szArglen[2] = strlen(szValue);
//        printf("lpush:%s\n", szValue);
//        redisAsyncCommandArgv((redisAsyncContext*)pRdsContext, RdsCbCmd_PUSH, NULL, iArgSize, ppArg, szArglen);
        redisAsyncCommand((redisAsyncContext*)pRdsContext, RdsCbCmd_PUSH, NULL,"LPUSH redislist_h %d", i);
//            redisReply* reply = (redisReply*)redisAsyncCommand((redisAsyncContext*)pRdsContext,NULL,NULL,"LLEN redislist_h");//interger
...
//    //}
        ...
        redisAsyncCommand((redisAsyncContext*)pRdsContext, RdsCbCmd, NULL,"brpop redislist_h 10");
}
...
void RdsCbCmd(redisAsyncContext* pRdsContext, void* pReply, void* pData) {
    ...
    if (REDIS_REPLY_ARRAY == pRdsReply->type) {
        //SendCustomRequest(pRdsContext);
        //printf("%u=======%u=======%s=======%s\n", pRdsReply->type,pRdsReply->elements, pRdsReply->element[0]->str, pRdsReply->element[1]->str);
        printf("POP:%s\n", pRdsReply->element[1]->str);
        //return;
    }

    //end.
    //printf("g_iCmdCallback : %d,%d\n",g_iCmdCallback,TEST_CMD_COUNT);
